...
MongoWireProtocol.prototype.parse = function parse(chunk) {
    ...
    let spec = this.specs[this.opCode];
    let chunkIncomplete = false
    let key;
    ...
    break_spec:
        for (key in spec) {
            ...
            const ptr = isArray && Array.isArray(this[key]) ? this[key] : [];
            ...
            switch (type) {
                case int32:
                    ...
                    this.index += 4;
                    if (key === 'flags') {
                        const flags = this.flagCodes[this.opCode];
                ...
                case document:
                    do {
                        const size = data.readInt32LE(this.index);
    ...
    for (const key in spec) {
        ...
        switch (type) {
            ...
            case document:
                ...
                for (let i = 0; i < documentsLength; i += 1) {
